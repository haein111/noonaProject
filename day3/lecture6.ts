...
function calculateDiscountedPrice(item: Product & Discount): number {
  const result = (item.price / 100) * (100 - item.discountPercentage); // discounted price
  return result;
}
...
function mergeUserData(
  ...
  const user = { ...profile, ...activity };
  return user;
}

// 사용자 요약 정보를 반환하는 함수
function getUserSummary(user: Profile & Activity): string {
  return `사용자 1 - ${profile.name} (${profile.email}) - 마지막 로그인: ${activity.lastLogin}`;
}
